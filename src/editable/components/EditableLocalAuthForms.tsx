...
const inputClass = 'h-[3.4rem] rounded-full border border-[color:var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 text-base font-semibold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.4rem] items-center justify-center rounded-full border border-[var(--slot4-page-text)] bg-[var(--slot4-page-text)] px-6 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--slot4-dark-text)] transition hover:border-[var(--slot4-accent)] hover:bg-[var(--slot4-accent)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.4rem] border px-4 py-3 text-sm font-semibold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
